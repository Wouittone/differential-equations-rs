...
macro_rules! file_algorithm {
    ($name:ident, $path:literal) => {
        define_explicit_rk_from_file!(pub $name, $path);
    };
}

algorithm!(MacroMethod);
define_explicit_rk_from_file!(pub FileMethod, "tableaux/file_method.toml");
file_algorithm!(WrappedFileMethod, "tableaux/wrapped_file_method.toml");
...
pub mod algorithms {
    pub use crate::native::{FileMethod, MacroMethod, NativeAlias, NativeMethod, WrappedFileMethod};
...
'@, $utf8NoBom)
    ...
    Assert-Contains $detected.rust_algorithm_implementation_names 'WrappedFileMethod' 'A wrapped file-backed proc-macro implementation was not detected.'
    ...
    foreach ($name in @('NativeMethod', 'MacroMethod', 'FileMethod', 'WrappedFileMethod', 'NativeAlias', 'SimpleNestedMethod')) {
