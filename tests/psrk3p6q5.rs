use differential_equations::{
    CallbackAction, OdeProblem, Psrk3p6q5, SaveMode, SolveError, SolveOptions, solve,
};
...
#[test]
fn psrk3p6q5_has_third_order_fixed_step_convergence() {
    let coarse = (solve(&exponential(), Psrk3p6q5, &fixed(0.1))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    let fine = (solve(&exponential(), Psrk3p6q5, &fixed(0.05))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
...
#[test]
fn psrk3p6q5_supports_forward_backward_save_at() {
...
#[test]
fn psrk3p6q5_callback_can_terminate_without_post_effect_rhs() {
    ...
    let solution = solve(&problem, Psrk3p6q5, &fixed(0.25)).unwrap();
    assert_eq!(solution.last_state(), &[42.0]);
    assert_eq!(solution.stats().callback_invocations, 1);
}

#[test]
fn psrk3p6q5_rejects_adaptive_configuration() {
    assert_eq!(
        solve(&exponential(), Psrk3p6q5, &SolveOptions::default()),
