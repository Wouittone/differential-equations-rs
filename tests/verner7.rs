use differential_equations::{CallbackAction, OdeProblem, SaveMode, SolveOptions, Vern7, solve};
...
fn generated_vern7_has_seventh_order_fixed_convergence() {
    ...
    assert!(e2 < e1 / 80.0, "errors {e1} and {e2}");
}
...
fn adaptive_backward_and_callback_paths_remain_safe() {
    ...
    let result = solve(&backward, Vern7, &options).unwrap();
