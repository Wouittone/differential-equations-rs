...
use differential_equations::{OdeProblem, SaveMode, SolveOptions, SspRk932, solve};
...
fn allocations_for(step: f64) -> usize {
    ...
    let solution = solve(&problem, SspRk932, &options).unwrap();
...
fn callback_free_fixed_steps_do_not_allocate_per_step() {
    ...
    assert!(
        one_step <= 7,
        "unexpected SSPRK932 allocation count: {one_step}"
    );
}
