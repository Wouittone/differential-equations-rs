...
fn exponential_problem(span: (f64, f64), initial: f64) -> Problem {
...
fn fixed_step_order_and_backward_integration() {
    let coarse = solve(&exponential_problem((0.0, 1.0), 1.0), Rodas4P2, &fixed(0.1))
        .unwrap()
        .last_state()[0];
    let fine = solve(
    ...
    let endpoint = solve(&backward, Rodas4P2, &fixed(0.02))
        .unwrap()
        .last_state()[0];
