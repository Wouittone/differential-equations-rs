...
fn force(_: f64, y: &[f64], d: &mut [f64]) -> Result<(), Infallible> {
    ...
    d[0]=y[2]; d[1]=y[3]; d[2]=-y[0]*inverse_cube; d[3]=-y[1]*inverse_cube;
    Ok(())
}
fn jacobian(_: f64, y: &[f64], j: &mut [f64]) -> Result<(), Infallible> {
    j.fill(0.0); j[2]=1.0; j[7]=1.0;
    let r2=y[0]*y[0]+y[1]*y[1];
...
fn orbit(step: f64, adaptive: Option<(f64,ErrorNorm,bool)>, numerical: bool) -> (f64,usize) {
    ...
    let mut solver=RosenbrockStepper::new(Rodas4.tableau().unwrap(),0.0,
        &[1.0-eccentricity,0.0,0.0,((1.0+eccentricity)/(1.0-eccentricity)).sqrt()]).unwrap();
    ...
    if adaptive.is_some_and(|x| x.2) { config.beta=[0.7/4.0,0.4/4.0,0.0]; }
    ...
    for _ in 0..100_000 {
        ...
        let mut zero_partial=|_:f64,_:&[f64],d:&mut[f64]|{d.fill(0.0);Ok::<_,Infallible>(())};
        let mut analytic_jacobian = jacobian;
        let view=solver.attempt(h,&mut force,
            if numerical {None} else {Some(&mut analytic_jacobian)},
            if numerical {None} else {Some(&mut zero_partial)}).unwrap();
        let accept=if let Some((_,norm,_))=adaptive {
            let error=tolerances.error_norm(view.previous_state,view.candidate,view.component_error.unwrap(),norm).unwrap();
            let decision=controller.assess(h,error).unwrap(); proposal=decision.next_step; decision.accepted
        } else {true};
        if accept {solver.accept().unwrap();} else {solver.reject().unwrap();}
    }
    assert_eq!(solver.time(),end);
    let mut anomaly=end;
    for _ in 0..10 { anomaly-=(anomaly-eccentricity*anomaly.sin()-end)/(1.0-eccentricity*anomaly.cos()); }
    let scale=(1.0-eccentricity*eccentricity).sqrt();
    let radius=1.0-eccentricity*anomaly.cos();
    let exact=[anomaly.cos()-eccentricity,scale*anomaly.sin(),-anomaly.sin()/radius,scale*anomaly.cos()/radius];
    let error=solver.state().iter().zip(exact).map(|(a,b)|(a-b).powi(2)).sum::<f64>().sqrt();
    (error,solver.statistics().rhs_evaluations)
}

#[test]
fn kepler_orbit_has_fourth_order_with_analytic_and_numerical_jacobians() {
    for numerical in [false,true] { for direction in [-1.0,1.0] {
        let coarse=orbit(direction*0.025,None,numerical).0;
        let fine=orbit(direction*0.0125,None,numerical).0;
        assert!(coarse/fine>12.0 && coarse/fine<22.0,"numeric={numerical} direction={direction} errors={coarse:e},{fine:e}");
    }}
}

#[test]
fn tightening_tolerance_improves_orbits_across_norm_and_controller_choices() {
    for norm in [ErrorNorm::Max,ErrorNorm::Rms] { for pi in [false,true] {
