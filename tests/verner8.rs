use differential_equations::{CallbackAction, OdeProblem, SaveMode, SolveOptions, Vern8, solve};
...
#[test]
fn generated_vern8_has_eighth_order_fixed_convergence() {
    ...
    let coarse = solve(&exponential(), Vern8, &options(0.5)).unwrap();
    let fine = solve(&exponential(), Vern8, &options(0.25)).unwrap();
    let exact = 2.0f64.exp();
    let e1 = (coarse.last_state()[0] - exact).abs();
    let e2 = (fine.last_state()[0] - exact).abs();
    assert!(e2 < e1 / 120.0, "errors {e1} and {e2}");
}
...
fn adaptive_backward_and_callback_paths_remain_safe() {
    ...
    let adaptive = solve(&exponential(), Vern8, &options).unwrap();
    ...
    let result = solve(&backward, Vern8, &options).unwrap();
