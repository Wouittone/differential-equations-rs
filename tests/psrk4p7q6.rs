...
#[test]
fn psrk4p7q6_has_fourth_order_fixed_step_convergence() {
    let coarse = (solve(&exponential(), Psrk4p7q6, &fixed(0.1))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    let fine = (solve(&exponential(), Psrk4p7q6, &fixed(0.05))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    assert!(coarse / fine > 12.0, "expected fourth-order convergence");
}

#[test]
fn psrk4p7q6_supports_forward_backward_save_at() {
...
#[test]
fn psrk4p7q6_callback_can_terminate_without_post_effect_rhs() {
    ...
    let solution = solve(&problem, Psrk4p7q6, &fixed(0.25)).unwrap();
    assert_eq!(solution.last_state(), &[42.0]);
    assert_eq!(solution.stats().callback_invocations, 1);
}

#[test]
fn psrk4p7q6_rejects_adaptive_configuration() {
    assert_eq!(
        solve(&exponential(), Psrk4p7q6, &SolveOptions::default()),
