...
use differential_equations::{OdeProblem, Rodas5, SaveMode, SolveOptions, solve};
...
#[test]
fn rodas5_callback_free_steps_do_not_allocate_per_step() {
    let one_step = allocations(1.0);
    let many_steps = allocations(0.001);
    assert_eq!(many_steps, one_step);
    assert!(
        one_step <= 25,
        "unexpected Rodas5 solve allocation count: {one_step}"
    );
}
