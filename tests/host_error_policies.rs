        ...
        for (tolerance, relative) in [(1e-3, 1e-9), (1e-6, 1e-12)] {
            for direction in [-1.0, 1.0] {
                let mut solver = ExplicitRungeKuttaStepper::new(
                    Tsit5.tableau().unwrap(),
                    0.0,
                    &[7e6, 0.0, 1.0, 0.0, 0.0, 1.0, 2.0, -3.0],
                )
                .unwrap();
                let tolerances = Tolerances::scalar(8, tolerance, relative)
                    .unwrap()
                    .with_block(2..6, tolerance * 0.5, relative * 0.5)
                    .unwrap()
                    .with_block(6..8, tolerance * 2.0, relative)
                    .unwrap();
                ...
                for _ in 0..10000 {
                    ...
                    let error = tolerances
                        .error_norm(&old, view.candidate, view.component_error.unwrap(), norm)
                        .unwrap();
                    if error <= 1.0 {
                ...
                for (&actual, expected) in solver.state().iter().zip(exact) {
                    assert!(
                        (actual - expected).abs() < (tolerance + relative * expected.abs()) * 60.0,
...
fn host_decisions_match_independent_max_and_rms_formulas() {
    ...
    for (absolute, relative) in [(1e-3, 1e-9), (1e-6, 1e-12)] {
        ...
        let scaled: Vec<_> = (0..7)
            .map(|i| error[i].abs() / (absolute + relative * old[i].abs().max(new[i].abs())))
            .collect();
