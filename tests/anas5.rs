use differential_equations::{
    Anas5, CallbackAction, OdeProblem, SaveMode, SolveError, SolveOptions, solve,
};
...
#[test]
fn anas5_has_high_order_fixed_step_convergence() {
    let coarse = (solve(&exponential(), Anas5::default(), &fixed(0.1))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    let fine = (solve(&exponential(), Anas5::default(), &fixed(0.05))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    assert!(
        coarse / fine > 10.0,
        "expected at least fourth-order convergence"
    );
}

#[test]
fn anas5_supports_forward_backward_save_at() {
...
#[test]
fn anas5_callback_can_terminate_without_post_effect_rhs() {
    ...
    let solution = solve(&problem, Anas5::default(), &fixed(0.25)).unwrap();
    assert_eq!(solution.last_state(), &[42.0]);
    assert_eq!(solution.stats().callback_invocations, 1);
}

#[test]
fn anas5_rejects_adaptive_configuration() {
    assert_eq!(
        solve(&exponential(), Anas5::default(), &SolveOptions::default()),
