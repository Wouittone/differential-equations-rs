use differential_equations::{CallbackAction, Msrk6, OdeProblem, SaveMode, SolveOptions, solve};
...
#[test]
fn msrk6_has_sixth_order_fixed_step_convergence() {
    let coarse = (solve(&exponential(), Msrk6, &fixed(0.1))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    let fine = (solve(&exponential(), Msrk6, &fixed(0.05))
        .unwrap()
        .last_state()[0]
        - std::f64::consts::E)
        .abs();
    assert!(coarse / fine > 40.0, "expected sixth-order convergence");
}
...
#[test]
fn msrk6_callback_can_terminate_without_post_effect_rhs() {
    ...
    let solution = solve(&problem, Msrk6, &fixed(0.25)).unwrap();
