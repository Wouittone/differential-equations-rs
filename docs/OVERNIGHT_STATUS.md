...
| ROS3P family | Native three-stage third-order parabolic Rosenbrock tableau | 105 library tests plus ROS3P backward/Jacobian/callback/save_at coverage | Julia unavailable on coordinator; pinned/full rerun blocked by `JULIA-PATH-20260809` | Rust-reviewed and conflict-resolved/merged as `97b889f` (source wave `a6a837c`); Julia retry required |
| GRK4A family | Native four-stage fourth-order embedded Rosenbrock tableau | 105 library tests plus GRK4A stiff/backward/Jacobian/callback/save_at/allocation integrations | Julia unavailable on coordinator; pinned/full rerun blocked by `JULIA-PATH-20260809` | Rust-reviewed and conflict-resolved/merged as `bd81350` (source wave `0fc2909`); Julia retry required |
| GRK4T family | Native four-stage fourth-order Rosenbrock tableau from pinned GRK4TRodasTableau | 105 library tests plus GRK4T stiff/backward/Jacobian/callback/save_at/allocation integrations | Julia unavailable on coordinator; pinned/full rerun blocked by `JULIA-PATH-20260809` | Rust-reviewed and merged as `8c47b27` (source wave `335985b4`); Julia retry required |
| ROS34PRw family | Native four-stage third-order Rosenbrock-W tableau | 105 library tests plus ROS34PRw stiff/Jacobian/backward/callback/save_at/convergence/allocation integrations | Isolated worker pinned check and ROS34PRw suite passed; integrated full suite blocked by `JULIA-PATH-20260809` and has one pre-existing unrelated ROS2 fixed failure | Rust-reviewed and conflict-resolved/merged as `2ac0ab2` (source wave `69a034f`); investigate the pre-existing ROS2 fixture discrepancy separately |
| ROS34PW1b family | Native four-stage third-order Rosenbrock-W tableau | 106 library tests plus ROS34PW1b Jacobian/callback/save_at/backward/convergence/allocation coverage | Julia unavailable on coordinator; pinned/full rerun blocked by `JULIA-PATH-20260809` | Rust-reviewed and conflict-resolved/merged as `f3eb804` (source wave `f3ee5bc`); primary fixed-step order threshold corrected to third-order behavior |
...
inventory regeneration: pass; 349 source references and strict cross-checkout byte identity verified (108 implemented/tested, 237 missing)
```
