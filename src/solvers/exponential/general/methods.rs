...
use super::{Scheme, mat_vec, matrix_exp};

pub(super) struct StepResult {
    pub(super) state: Vec<f64>,
    pub(super) error: Vec<f64>,
    pub(super) current_nonlinear: Vec<f64>,
}

#[allow(clippy::too_many_arguments)]
pub(super) fn perform_step<F, P>(
    scheme: Scheme,
...
pub(super) fn phi_action(a: &[f64], scale_a: f64, order: usize, input: &[f64]) -> Vec<f64> {
    assert!(order > 0);
    let n = input.len();
    let size = n + order;
