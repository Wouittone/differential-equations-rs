...
use super::config::{JVODE, JvodeMethod};
...
pub(super) struct NordsieckKernel<F, P> {
    family: Family,
    config: JVODE,
    dimension: usize,
    maximum_order: usize,
...
impl<F, P> NordsieckKernel<F, P> {
    pub(super) fn an5(dimension: usize) -> Self {
        Self::new(Family::An5, JVODE::adams(), dimension, 5)
    }

    pub(super) fn jvode(dimension: usize, config: JVODE) -> Self {
