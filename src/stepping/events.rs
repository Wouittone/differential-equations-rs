...
use super::{AdaptiveController, ExplicitRungeKuttaStepper, IntegrationError, IntegrationOutcome,
...
impl Default for RootOptions {
    fn default() -> Self {
        Self { time_tolerance: 1e-10, value_tolerance: 0.0,
            maximum_iterations: 64, direction: RootDirection::Any }
    }
}
...
fn crosses(a: f64, b: f64, direction: RootDirection) -> bool {
    ...
    match direction { RootDirection::Any=>rising||falling,
        RootDirection::Rising=>rising,RootDirection::Falling=>falling }
}
...
pub fn integrate_rk_until_event<F,N,O,C,E>(
    stepper: &mut ExplicitRungeKuttaStepper<'_>, controller: &mut AdaptiveController,
    endpoint: f64, requested_times: &[f64], maximum_attempts: usize,
    options: RootOptions, rhs: &mut F, norm: &mut N, condition: &mut C, observer: &mut O,
) -> Result<RootOutcome,RootError<E>>
where F:FnMut(f64,&[f64],&mut[f64])->Result<(),E>,
    N:FnMut(&[f64],&[f64],&[f64])->Result<f64,E>,
    O:FnMut(Observation<'_>)->Result<ObserverAction,E>,
    C:FnMut(f64,&[f64])->Result<f64,E>,
{
    if !options.time_tolerance.is_finite() || options.time_tolerance<=0.0
        || !options.value_tolerance.is_finite() || options.value_tolerance<0.0
        || options.maximum_iterations==0 { return Err(RootError::Configuration); }
    if !endpoint.is_finite() {return Err(failure(StepFailure::NonFinite));}
    let start=stepper.time();
    let direction=(endpoint-start).signum();
    let mut previous=start;
    for &time in requested_times {
        if !time.is_finite() || (time-previous)*direction<0.0
            || (time-start)*direction<0.0 || (endpoint-time)*direction<0.0
    ...
    if !value.is_finite(){return Err(RootError::NonFiniteCondition);}
    let mut index=0;
    while requested_times.get(index)==Some(&start){index+=1;}
    let initial_root=value.abs()<=options.value_tolerance;
    if index>0 || initial_root || start==endpoint {
        let stop=observer(Observation {time:start,state:stepper.state(),requested:index>0,
            endpoint:start==endpoint}).map_err(application)?==ObserverAction::Stop;
        if stop || initial_root || start==endpoint {
            return Ok(RootOutcome {integration:IntegrationOutcome{time:start,
                interrupted:stop||initial_root,forced_acceptances:0},
    ...
    for _ in 0..maximum_attempts {
        ...
        if !next_value.is_finite(){stepper.reject().map_err(failure)?;return Err(RootError::NonFiniteCondition);}
        let mut trial_controller=controller.clone();
        let decision=match trial_controller.assess(h,error) {
            Ok(d)=>d, Err(e)=>{stepper.reject().map_err(failure)?;return Err(IntegrationError::Controller(e).into());}
        };
        ...
        if has_root && next_value.abs()>options.value_tolerance {
            ...
            for _ in 0..options.maximum_iterations {
                let mut middle=low+(high-low)*0.5;
                if start+middle==start {middle=high;}
                let error=stepper.attempt_with_norm(start+middle,middle,rhs,&mut |old,new,e| {
                    let error=norm(old,new,e)?;
                ...
                if next_value.abs()<=options.value_tolerance
                    ...
                    || start+middle==start+low || start+middle==start+high {
                    *controller=root_controller;
                    forced+=usize::from(root_decision.forced);
                    located=true;
                    break;
                }
                if crosses(low_value,next_value,RootDirection::Any){high=middle;}
                else {low=middle;low_value=next_value;}
                stepper.reject().map_err(failure)?;
            }
            if !located {
                if stepper.pending_step().is_some(){stepper.reject().map_err(failure)?;}
                // A controller rejection above supplies a valid retry; an
                // exhausted localization budget must be reported explicitly.
                if retry_adaptive {continue;}
        ...
        while requested_times.get(index)==Some(&stepper.time()){index+=1;}
        let done=stepper.time()==endpoint;
        let stopped=observer(Observation{time:stepper.time(),state:stepper.state(),requested,
            endpoint:done}).map_err(application)?==ObserverAction::Stop;
        if has_root || stopped || done {
            return Ok(RootOutcome{integration:IntegrationOutcome{time:stepper.time(),
                interrupted:has_root||stopped,forced_acceptances:forced},
